...
error_reporting(E_ALL); // Preserved for logging purposes, not displaying to user.
...
$userId = (int) $_SESSION['user_id']; // Cast user id to integer for safety

// Fetch all active (open) support tickets for the user using prepared statement.
$activeTickets = [];
if ($conn) {
    $stmt = $conn->prepare("SELECT cr.id AS room_id, c.id AS consultation_id, c.created_at
                            FROM chat_rooms cr
...
// Fetch closed tickets for the ticket history using prepared statement.
$closedTickets = [];
if ($conn) {
    $stmt = $conn->prepare("SELECT cr.id AS chat_room_id, c.id AS consultation_id, c.created_at, c.closed_at
                            FROM consultations c
                            JOIN chat_rooms cr ON c.id = cr.consultation_id
                            WHERE c.user_id = ? AND c.status = 'closed'
