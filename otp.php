...
<head>
  ...
  <style>
    ...
    #signup-section, #login-face-validation, #update-details-section {
      display: none;
    }
    ...
    #capturedFacePreview, #updateFaceCanvas, #updateCapturedFacePreview {
      display: none;
      max-width: 320px;
      margin-top: 10px;
...
<body>
    ...
    <form id="otpForm" class="w-75">
      ...
      <div id="update-details-section">
        ...
        <div class="mb-3">
          ...
          <input type="text" name="update_first_name" class="form-control" id="update_first_name" placeholder="Enter your first name">
        </div>
        <div class="mb-3">
          <label for="update_last_name" class="form-label">Last Name</label>
          <input type="text" name="update_last_name" class="form-control" id="update_last_name" placeholder="Enter your last name">
        </div>
        <div id="updateFaceCaptureSection" style="margin-top:20px;">
          <h4>Capture Your Face</h4>
          <video id="updateFaceVideo" width="320" height="240" autoplay muted style="border:1px solid #ccc;"></video>
          <br>
      ...
      <div id="login-face-validation">
        ...
        <img id="storedFacePreview" src="" alt="Stored Face Reference" style="max-width:320px; border:1px solid #ccc; margin-bottom:10px;">
        <h4>Capture Your Face</h4>
  ...
  <!-- Loading Screen -->
  <div id="loadingScreen" class="d-none position-fixed w-100 h-100 top-0 start-0 bg-white bg-opacity-75 d-flex justify-content-center align-items-center">
  ...
  <script>
    ...
    document.getElementById('resendOtpBtn').addEventListener('click', async () => {
      const emailInput = document.getElementById('email') ? document.getElementById('email').value : sessionStorage.getItem('email');
      if (!emailInput) {
    ...
    async function loadReferenceDescriptor() {
      ...
      try {
        const fullURL = window.location.origin + '/capstone-php/' + storedFaceImageURL;
        const img = new Image();
        ...
        const detection = await faceapi.detectSingleFace(img, new faceapi.TinyFaceDetectorOptions({ inputSize: 416, scoreThreshold: 0.5 }))
          .withFaceLandmarks()
          .withFaceDescriptor();
        if (detection) {
    ...
    function setVisuallyImpaired(isImpaired, modalInstance) {
      fetch('backend/routes/set_visually_impaired.php', {
          ...
          body: JSON.stringify({ visually_impaired: isImpaired })
        })
