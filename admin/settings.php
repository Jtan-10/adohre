...
<body>
    <div class="d-flex">
        ...
        <div class="container mt-5">
            ...
            <p class="text-muted">Find and optionally delete files in S3 under <code>uploads/</code> that aren’t
                referenced in the database. Start with a dry run (List Orphans), then delete if you’re sure.</p>
    ...
    <script nonce="<?= $cspNonce ?>">
        ...
        // Security Settings form submission removed
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const callCleanup = async (mode = 'list', extra = {}) => {
                ...
                if (!res.ok) {
                    // Normalize error shape so UI can display it
            ...
            document.getElementById('btnListOrphans').addEventListener('click', async () => {
                try {
                    document.getElementById('orphansSummary').innerHTML =
                        '<div class="alert alert-secondary">Scanning S3 for orphans…</div>';
                    const data = await callCleanup('list');
                    if (data && data.status) {
                        renderOrphans(data.orphans || []);
                        showApiMessage('Scan completed.', 'success');
                    } else {
                        showApiMessage((data && data.message) || 'Scan failed.', 'danger');
                    }
                } catch (e) {
                    console.error(e);
            ...
            document.getElementById('btnDeleteOrphans').addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (data && data.status) {
                        ...
                        showApiMessage((data && data.message) || 'Delete failed.', 'danger');
                    }
                } catch (e) {
                    console.error(e);
